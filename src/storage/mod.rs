...
pub struct StorageEngine {
    _path: PathBuf,
    file: File,
    index: HashMap<String, u64>, // key to file offset
}
...
impl StorageEngine {
    pub fn new(path: impl Into<PathBuf>) -> Result<Self> {
        ...
        Ok(Self { _path: path, file, index })
    }

    pub fn set(&mut self, key: String, value: Vec<u8>) -> Result<()> {
        let record = Record { key: key.clone(), value, deleted: false };
    ...
    pub fn delete(&mut self, key: &str) -> Result<()> {
        if let Some(_) = self.index.remove(key) {
            let record = Record { key: key.to_string(), value: vec![], deleted: true };
...
mod tests {
    ...
    fn test_storage_basic() {
        ...
        engine.set("key1".to_string(), b"val1_updated".to_vec()).unwrap();
